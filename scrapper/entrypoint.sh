...
aws dynamodb describe-table --region $REGION --endpoint-url $DYNAMO_ENDPOINT --table-name $GAMES_TABLE_NAME > /dev/null 2>&1
if [ $? -ne 0 ]; then
    ...
    echo "Table $GAMES_TABLE_NAME already exists."
fi
